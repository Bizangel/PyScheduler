from scheSpanish import * #import very own defs
...
filemenu.add_command(label="Nuevo                              Ctrl+N", command=lambda: new_file(canvas,classdict,unsaved,invoke_save,schedulepath,filename))
root.bind('<Control-n>',lambda event: new_file(canvas,classdict,unsaved,invoke_save,schedulepath,filename) )
filemenu.add_command(label="Abrir                                 Ctrl+O", command=lambda: open_file(canvas,classdict,reference_channel,unsaved,invoke_save,schedulepath,filename) )
root.bind('<Control-o>',lambda event: open_file(canvas,classdict,reference_channel,unsaved,invoke_save,schedulepath,filename))
filemenu.add_command(label="Guardar                            Ctrl+S", command=lambda: save_file(classdict,schedulepath,save_state,invokebutton))
root.bind('<Control-s>',lambda event: save_file(classdict,schedulepath,save_state,invokebutton) )
filemenu.add_command(label="Guardar Como...     Ctrl+Alt+S", command=lambda: save_as_file(classdict,save_state,invokebutton,filename,schedulepath))
...
showmenu.add_checkbutton(label='Titulo Horario',onvalue=1, offvalue=False, variable=show_title,command = lambda: update_title_show(show_title,title_label))
showmenu.add_checkbutton(label='Tiempos de Clase',onvalue=1, offvalue=False, variable=show_timestamp,command = lambda: update_timestamp_show(show_timestamp,classdict))

menubar.add_cascade(label="Mostrar", menu=showmenu)

newclassButton = tk.Button(root,text='Añadir Materia',command=lambda: open_class_window(canvas,classdict,reference_channel) )
...
urosario_fetch = tk.Button(root,text='Añadir de la web...',command= lambda: UrosarioPopUp(canvas,classdict,reference_channel) )
...
label = tk.Label(canvas,text=' Lunes        Martes       Miércoles     Jueves       Viernes      Sábado',font=('ComicSans',16))
