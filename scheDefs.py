...
from urosariofetch import UrosarioFetch as get_class_data
import requests
...
def new_file(canvas,classdict,unsaved,invoke_save,schedulepath,filename):
    ...
    initpath = get_path()
    initpath += '/schedules'
    filepath=  filedialog.asksaveasfilename(initialdir = initpath,title = "New File...",filetypes = (("Schedule Files","*.txt"),("All Files","*.*")),
                                            initialfile='new_schedule',defaultextension='*.txt')
    if filepath == '': return 
    #gets path
...
def open_file(canvas,classdict,reference_channel,unsaved,invoke_save,schedulepath,filename):
    ...
    initpath += '/shedules'
    path = filedialog.askopenfilename(initialdir =initpath,title = "Select file",filetypes = (("Schedule Files","*.txt"),("All Files","*.*")))
...
def open_schedule(path,canvas,classdict,reference_channel):
    ...
    try:
        ...
        error_popup('Could not open file')
        return
...
def save_as_file(classdict,save_state,invokebutton,filename,schedulepath):
    ...
    initpath += '\\schedules'
    filepath=  filedialog.asksaveasfilename(initialdir = initpath,title = "Select file",filetypes = (("Schedule Files","*.txt"),("All Files","*.*")),
...
class classSquare:
    def __init__(self,canvas,classdict,dayid,start,end,title,reference_channel,color='red'):
        ...
        self.popup_menu.add_command(label="Add/Edit Teacher",
    ...
    def update_display_label(self):
        ...
        if len(textlabel) > 50: self.label['font'] = ('ComicSans',7)
...
class Subject: #class info format is a list of tuple trios containing (<day>,<start>,<end>) ex: [(2,9,11),(4,9,11)]  this means class on WED and FRI from 9-11
    ...
    def delete(self,canvas,classdict,noask=False):
        ...
        if noask:
            ...
            return
        #else keep going
        if messagebox.askokcancel("Deletion", "Are you sure you want to delete {0}?".format(self.title)):
            for square in self.squares: #deletes all square objects
                square.delete(canvas,classdict)
            del classdict[self.title] #deletes class entry from dictionary
            temp = self.reference_channel[1].get()
...
def createClass(canvas,classdict,title,classinfo,reference_channel,n_credits,color='red'): #check subject constructor for classinfo parameters and style
    ...
    if title in classdict.keys():
        error_popup('A class by that name already exists')
...
class scheduleWindow:
    def __init__(self,canvas,classdict,reference_channel):
        ...
        self.colorButton = tk.Button(self.window,text='Change Color',command=lambda:get_color(self.color,self.colordisplay))
        self.colorButton.place(relx=0.98,rely=0.06,anchor='e')
        ...
        self.addblock_button = tk.Button(self.window,text='Add Block',command=lambda:self.add_class_block())
        self.addblock_button.place(relx=0.77,rely=0.15)

        self.weekday = tk.Listbox(self.window)
        self.weekday.insert(1,'Monday')
        self.weekday.insert(2,'Tuesday')
        self.weekday.insert(3,'Wednesday')
        self.weekday.insert(4,'Thursday')
        self.weekday.insert(5,'Friday')
        self.weekday.insert(6,'Saturday')
        ...
        self.createButton = tk.Button(self.window,text='Create!',bg='lightgray',command= lambda:self.send_class_info() )
        ...
        a = tk.Label(self.window,text='Name:',font=('ComicSans',12))
        a.place(relx = 0,rely=0.02)
        self.class_block_display = tk.Label(self.window,text='--- Add Class Block --- /// --- /// ---',font=('ComicSans',12))
        self.class_block_display.place(relx = 0,rely=0.15)
        a = tk.Label(self.window,text='Start Time',wraplength=60,font=('ComicSans',12))
        a.place(relx=0.07,rely=0.265)
        a = tk.Label(self.window,text='End Time',wraplength=60,font=('ComicSans',12))
    ...
    def send_class_info(self):
        ...
        if len(self.class_blocks) == 0:
            error_popup('Input at least one class block')
            return
        elif self.entrytitle.get() == '':
            error_popup('Input a subject name')
            return
        classinfo_list = []
    ...
    def add_class_block(self):
        ...
        if len(daystring) == 0:
            messagebox.showwarning('Missing Info','Please select a day')
            return
        ...
        if len(starthour) == 0 or len(endhour) == 0:
            messagebox.showwarning('MissingInfo','Please input start and end time')
            return 
        ...
        if (int(starthour) < 7 and start_ampm == 'AM') or ( 12 > int(starthour) > 7 and start_ampm == 'PM')or (int(starthour) == 12 and start_ampm=='AM'):
            error_popup('No starting times before 7AM or past 7PM will be accepted')
            return
        elif (int(endhour) < 7 and end_ampm == 'AM') or (12 > int(endhour) > 7 and end_ampm == 'PM') or (int(endhour) == 12 and end_ampm=='AM'):
            error_popup('No ending times before 7AM or past 7PM will be accepted')
            return
        ...
        if hrto24format(startstring) >= hrto24format(endstring):
            error_popup('Starting time can\'t be greater or equal than ending time')
            return
        ...
        if overlap_test(blockstring,self.pre_existant) or overlap_test(blockstring,self.class_blocks):
            if messagebox.askokcancel("Overlap Warning", "Adding this block will cause overlapping with another class block \n Continue? "):
        ...
        if blockstring not in self.class_blocks:
            ...
            error_popup('A Class Block with those times already exists')
...
class EditWindow(scheduleWindow):
    def __init__(self,canvas,classdict,reference_channel,title):
        ...
        self.window.title('Edit Subject')
        ...
        self.class_block_display['text'] = '--- Edit Class Block --- /// --- /// ---'
...
class TeacherPopUp:
    def __init__(self,canvas,classdict,title):
        ...
        if len(prevteacher) == 0:self.window.title('Add Teacher')
        else: self.window.title('Edit Teacher')
        ...
        self.button = tk.Button(self.window,text='Confirm',command=lambda: self.return_teacher_name(classdict,title) )
...
def asksave():
    ...
    answer = messagebox.askyesnocancel('Confirmation','You have unsaved changes, do you want to save?')
...
##    name = requests.get('https://urapprest.uranalytics.p.azurewebsites.net/api/asignaturaDetalle?codigo={0}&opcionDetalle=DetalleActividad'.format(code),header=head)
##    stringvar.set('Fetching Class Credits...')
##    c_credits = requests.get('https://urapprest.uranalytics.p.azurewebsites.net/api/asignaturaDetalle?codigo={0}&opcionDetalle=DetalleAsignatura'.format(code),header=head)
##    stringvar.set('Fetching Class Groups...')
##    group = requests.get('https://urapprest.uranalytics.p.azurewebsites.net/api/asignaturaDetalle?codigo={0}&opcionDetalle=DetalleGrupo&codActividad={0}'.format(code),header=head)             
##
##    json_name = name.json()
...
##        stringvar.set('Couldn\' fetch class credits')
...
##        grupinfo = requests.get('https://urapprest.uranalytics.p.azurewebsites.net/api/asignaturaDetalle?codigo={0}&opcionDetalle=DetalleFecha&codActividad={0}&codGrupo={1}'.format(code,grupid),header=head)
...
##        r = requests.get('https://urapprest.uranalytics.p.azurewebsites.net/api/asignaturaDetalle?codigo={0}&opcionDetalle=DetalleHorario&codActividad={0}&codGrupo={1}&fechaIni={2}&fechaFin={3}'.format(code,grupcod,ini,fin),header=head )
...
##    if not fetched_credits: warning = 'Note: Couldn\t fetch class credits'
##    else: warning = ''
##    stringvar.set('Done!'+'\n'+'\n'.join(classinfo)+'\n'+warning)
...
class UrosarioPopUp:
    def __init__(self,canvas,classdict,reference_channel):
        ...
        self.window.grab_set()
        self.window.geometry('300x300+900+300')
        self.window.title('Add from Database...')
        self.window.resizable(False,False)
        ...
        self.entrygroup.place(anchor='n',relx=0.65,rely=0.275,relwidth=0.1)

        self.color = tk.StringVar()
        self.color.set('#ff0000')
        self.colorButton = tk.Button(self.window,text='Set Color',command=lambda:get_color(self.color,self.colordisplay))
        self.colorButton.place(relx=0.1,rely=0.4,anchor='nw')
        self.colordisplay = tk.Label(self.window,bg=self.color.get(),relief='raised')
        self.colordisplay.place(relx=0.35,rely=0.405,relwidth=0.07,relheight=0.07)

        self.fetchButton = tk.Button(self.window,text='Fetch!',command=lambda:self.fetch_data())
        ...
        self.addbutton = tk.Button(self.window,text='Add!',state='disabled',command=lambda: self.send_class_info() )
        self.addbutton.place(anchor='s',relx=0.5,rely=1)


        self.extraVar = tk.IntVar()
        self.extrabox = tk.Checkbutton(self.window,text='Extras? Labs... etc.',variable = self.extraVar)
        self.extrabox.select()
        self.extrabox.place(relx=1,rely=0,anchor='ne')


        a = tk.Label(self.window,text='Subject\n Code ',font=('ComicSans',16))
        a.place(relx=0.1,rely=0.05)
        a = tk.Label(self.window,text='Group Number:  ',font=('ComicSans',16))
        a.place(relx=0.05,rely=0.25)
    def fetch_data(self):
        try:
            self.display.set('Fetching data')
            if self.extraVar.get() == 1:extras = True
            else: extras = False
            self.title,self.teacher,self.c_credits,self.class_blocks = get_class_data(int(self.entrycode.get()),int(self.entrygroup.get()),extras,self.display,self.window)
            self.addbutton['state'] = 'normal'
            self.teacher = beauty_string(self.teacher)
            self.title = beauty_string(self.title)
        except:
            self.display.set('Could not fetch class info, try again.')
    def send_class_info(self):
        for blockstring in self.class_blocks:
