...
def beauty_string(string):
    '''Receives a string and makes that every first letter of a word is caps and all other are lower,
    returns new string '''
    wordlist = string.split(' ')
    remaked = []
    for word in wordlist:
        if len(word) > 1:
            remaked.append(word[0].upper() + word[1:].lower())
        else:
            remaked.append(word.upper())
...
def open_file(canvas, classdict, reference_channel, unsaved, invoke_save, schedulepath, filename):
    ...
    if unsaved.get():
        ...
        if answer == 0:
            ...
            pass  # continue and don't save
        elif answer == 2:
            invoke_save.invoke()  # saves

    initpath = get_path()  # gets folder path
    initpath += '/shedules'
    path = filedialog.askopenfilename(initialdir=initpath, title="Escoger Archivo...", filetypes=(
        ("Archivos de Horario", "*.txt"), ("Todos", "*.*")))
    if path == '':
        return  # Do nothing if no path given
...
def daystring_index(daystring):
    ...
    if daystring == 'LU' or daystring == 'MON':
        return 0
    elif daystring == 'MA' or daystring == 'TUE':
        return 1
    elif daystring == 'MI' or daystring == 'WED':
        return 2
    elif daystring == 'JU' or daystring == 'THU':
        return 3
    elif daystring == 'VI' or daystring == 'FRI':
        return 4
    elif daystring == 'SA' or daystring == 'SAT':
        return 5
    elif daystring == 'DO' or daystring == 'SUN':
        return 6

    # full names
    daystring = daystring.lower()
    if daystring == 'lunes' or daystring == 'monday':
        return 0
    elif daystring == 'martes' or daystring == 'tuesday':
        return 1
    elif daystring == 'miercoles' or daystring == 'wednesday' or daystring == 'miércoles':
        return 2
    elif daystring == 'jueves' or daystring == 'thursday':
        return 3
    elif daystring == 'viernes' or daystring == 'friday':
        return 4
    elif daystring == 'sabado' or daystring == 'saturday' or daystring == 'sábado':
        return 5
    elif daystring == 'domingo' or daystring == 'sunday':
        return 6
    else:
...
def floathour_to_string(floathour):
    ...
    hour = int(hour)
    if minutes != 0:
        return str(hour)+':'+str(minutes)+ampm
    else:
        return str(hour) + ':' + '00' + ampm  # ensure double 00
...
def canvasRect(canvas, topleft, width, height, color='red'):
    ...
    left = topleft[0]
    top = topleft[1]
    poly = canvas.create_polygon(topleft, (left+width, top),
                                 (left+width, top+height), (left, top+height), fill=color)
    return poly
...
class classSquare:
    def __init__(self, canvas, classdict, dayid, start, end, title, reference_channel, color='red'):
        ...
        self.label = tk.Label(canvas, text=textlabel, bg=color, wraplength=110,
                              fg=contrastcolor, font=('ComicSans', fontsize))
        ...
        self.infostring = index_to_daystring(
            dayid) + ': ' + floathour_to_string(start) + '-' + floathour_to_string(end)
    ...
    def update_display_label(self):
        ...
        if self.show_timestamp:
            textlabel += self.timestamp + '\n'
        textlabel += self.teacher
        self.label['text'] = textlabel
        if len(textlabel) > 80:
            self.label['font'] = ('ComicSans', 7)
        elif len(textlabel) > 50:
            self.label['font'] = ('ComicSans', 8)
        elif len(textlabel) > 30:
            self.label['font'] = ('ComicSans', 10)
        else:
            self.label['font'] = ('ComicSans', 13)


# class info format is a list of tuple trios containing (<day>,<start>,<end>) ex: [(2,9,11),(4,9,11)]  this means class on WED and FRI from 9-11
class Subject:
    def __init__(self, canvas, classdict, title, classinfo, reference_channel, n_credits, color='red'):
        ...
        for info in classinfo:  # create classsquares
            ...
            self.squares.append(classSquare(canvas, classdict, dayid, start,
                                            end, title, reference_channel, color))
...
def hrto24format(hourstring):
    ...
    if len(hourstring) == 6:
        short = True  # short means only 1 digit on start before : like 8:00AM
    else:
        short = False  # not short more than 1, like 12:00AM

    time = 0
    ampm = hourstring[-2:]
    if ampm.upper() == 'PM':
        if short:
            time = int(hourstring[0])+12
        else:
            if hourstring[0:2] == '12':
                time = 12
            else:
                time = int(hourstring[0:2])+12
    else:
        if short:
            time = int(hourstring[0])
        else:
            time = int(hourstring[0:2])

    if short:
        time += int(hourstring[2:4])/60
    else:
        time += int(hourstring[3:5])/60
...
# check subject constructor for classinfo parameters and style
def createClass(canvas, classdict, title, classinfo, reference_channel, n_credits, color='red'):
...
class scheduleWindow:
    def __init__(self, canvas, classdict, reference_channel):
        ...
        self.startminutes = tk.Spinbox(self.window, values=(
            '00', '15', '30', '45'), state='readonly', wrap=True)
        ...
        self.starthour['validatecommand'] = (
            self.starthour.register(self.inthourcheck), '%P', '%d', '%W')
        self.starthour.place(relx=0.25, rely=0.3, relwidth=0.1)

        self.endminutes = tk.Spinbox(self.window, values=(
            '00', '15', '30', '45'), state='readonly', wrap=True)
        ...
        self.entrycredits['validatecommand'] = (
            self.entrycredits.register(self.intcreditscheck), '%P', '%d')
        self.entrycredits.place(anchor='sw', relx=0.2, rely=1, relwidth=0.1)

        # --- /// --- /// --- Color Picker
        self.color = tk.StringVar()
        self.color.set('#ff0000')
        self.colorButton = tk.Button(self.window, text='Cambiar Color',
                                     command=lambda: get_color(self.color, self.colordisplay))
        ...
        self.start_ampm = tk.Spinbox(self.window, values=(
            'AM', 'PM'), state='readonly', wrap=True, name='aname')
        ...
        self.addblock_button = tk.Button(
            self.window, text='Agregar Bloque', command=lambda: self.add_class_block())
        ...
        self.createButton = tk.Button(self.window, text='Crear!',
                                      bg='lightgray', command=lambda: self.send_class_info())
        ...
        self.class_block_display = tk.Label(
            self.window, text='--- Añadir Bloques --- /// --- /// -', font=('ComicSans', 12))
    ...
    def send_class_info(self):
        ...
        if len(class_credits) == 0:
            class_credits = 0
        class_credits = int(class_credits)
        createClass(self.canvas, self.classdict, self.entrytitle.get(), classinfo_list,
                    self.reference_channel, class_credits, self.color.get())
        self.window.destroy()
    ...
    def add_class_block(self):
        ...
        blockstring = daystring + ': ' + starthour + ':' + startminutes + \
            start_ampm + '-' + endhour + ':' + endminutes + end_ampm
        ...
        if blockstring not in self.class_blocks:
            ...
            self.popup_menu.add_command(
                label=blockstring, command=lambda: self.delete_block(blockstring))
        else:
    ...
    def inthourcheck(self, string, command, widgetname):
        ...
        try:
            ...
            if self.toplevelid == 1:
                insertstring = ''
            else:
                insertstring = str(self.toplevelid)
            ...
            if widgetname == widgetcheck1:  # means start_hour
                if n >= 7 and n != 12:
                    setampm(self.start_ampm, 'AM')
                elif n < 7 or n == 12:
                    setampm(self.start_ampm, 'PM')
            elif widgetname == widgetcheck2:  # means end_hour
                if n >= 7 and n != 12:
                    setampm(self.end_ampm, 'AM')
                elif n < 7 or n == 12:
                    setampm(self.end_ampm, 'PM')
        except:
            pass
...
class EditWindow(scheduleWindow):
    def __init__(self, canvas, classdict, reference_channel, title):
        ...
        # 0 is title 1 is color ,2 is teacher, 3 are credits so start from 4 and on
        self.class_blocks = classinfo[4:]
...
class TeacherPopUp:
    def __init__(self, canvas, classdict, title):
        ...
        if len(prevteacher) == 0:
            self.window.title('Añadir Profesor')
        else:
            self.window.title('Editar Profesor')
        ...
        self.button = tk.Button(self.window, text='Confirmar',
                                command=lambda: self.return_teacher_name(classdict, title))
...
def get_class_data(code, groupid, stringvar=None):
    ...
    name = requests.get(
        'https://urapprest.uranalytics.p.azurewebsites.net/api/asignaturaDetalle?codigo={0}&opcionDetalle=DetalleActividad'.format(code))
    stringvar.set('Fetching Class Credits...')
    c_credits = requests.get(
        'https://urapprest.uranalytics.p.azurewebsites.net/api/asignaturaDetalle?codigo={0}&opcionDetalle=DetalleAsignatura'.format(code))
    stringvar.set('Fetching Class Groups...')
    group = requests.get(
        'https://urapprest.uranalytics.p.azurewebsites.net/api/asignaturaDetalle?codigo={0}&opcionDetalle=DetalleGrupo&codActividad={0}'.format(code))
    ...
    for group in grouplist:
        ...
        grupinfo = requests.get(
            'https://urapprest.uranalytics.p.azurewebsites.net/api/asignaturaDetalle?codigo={0}&opcionDetalle=DetalleFecha&codActividad={0}&codGrupo={1}'.format(code, grupid))
        grupinfo = grupinfo.json()
        group_dates.append((grupinfo['data'][-1]['fechaInicio'],
                            grupinfo['data'][-1]['fechaFin']))  # save last schedule
    ...
    for i in range(len(grouplist)):
        ...
        r = requests.get(
            'https://urapprest.uranalytics.p.azurewebsites.net/api/asignaturaDetalle?codigo={0}&opcionDetalle=DetalleHorario&codActividad={0}&codGrupo={1}&fechaIni={2}&fechaFin={3}'.format(code, grupcod, ini, fin))
    ...
    if not fetched_credits:
        warning = 'Nota: No se pudo obtener los créditos de la clase'
    else:
        warning = ''
    stringvar.set(name+'\n'+'\n'.join(classinfo)+'\n'+warning)
...
class UrosarioPopUp:
    def __init__(self, canvas, classdict, reference_channel):
        ...
        self.entrygroup['validatecommand'] = (
            self.entrygroup.register(self.groupintcheck), '%P', '%d')
        self.entrygroup.place(anchor='n', relx=0.7, rely=0.275, relwidth=0.1)

        self.color = tk.StringVar()
        self.color.set('#ff0000')
        self.colorButton = tk.Button(self.window, text='Color',
                                     command=lambda: get_color(self.color, self.colordisplay))
        ...
        self.fetchButton = tk.Button(self.window, text='Obtener!',
        ...
        self.addbutton = tk.Button(self.window, text='Añadir!',
                                   state='disabled', command=lambda: self.send_class_info())
    ...
    def fetch_data(self):
        try:
            ...
            self.title, self.teacher, self.c_credits, self.class_blocks = get_class_data(
                int(self.entrycode.get()), int(self.entrygroup.get()), self.display)
    ...
    def send_class_info(self):
        ...
        createClass(self.canvas, self.classdict, self.title, classinfo_list,
                    self.reference_channel, int(self.c_credits), self.color.get())
...
# ,invokebutton] #simple list, allows easy communication between everything, 0 is widget tracker and 1 is n_credits var, 2 bool var of show_timestamp
reference_channel = [0, credits_tracker, show_timestamp, 1]
...
try:
    ...
    error_popup('No se pudo restaurar el calendario previo, favor crear nuevo')
...
save_state.set(get_current_save_state(classdict))
invokebutton = tk.Button(root, command=lambda: process_title_save(
    root, classdict, save_state, schedulepath, unsaved))
...
root.protocol('WM_DELETE_WINDOW', lambda: on_exit(root, show_title,
                                                  show_timestamp, schedulepath, unsaved, invoke_save))
...
invoke_save = tk.Button(command=lambda: save_file(
    classdict, schedulepath, save_state, invokebutton))

menubar = tk.Menu(canvas)

filemenu = tk.Menu(menubar, tearoff=0)
filemenu.add_command(label="Nuevo                              Ctrl+N",
                     command=lambda: new_file(canvas, classdict, unsaved, invoke_save, schedulepath, filename))
root.bind('<Control-n>', lambda event: new_file(canvas, classdict,
                                                unsaved, invoke_save, schedulepath, filename))
filemenu.add_command(label="Abrir                                 Ctrl+O", command=lambda: open_file(
    canvas, classdict, reference_channel, unsaved, invoke_save, schedulepath, filename))
root.bind('<Control-o>', lambda event: open_file(canvas, classdict,
                                                 reference_channel, unsaved, invoke_save, schedulepath, filename))
filemenu.add_command(label="Guardar                            Ctrl+S",
                     command=lambda: save_file(classdict, schedulepath, save_state, invokebutton))
root.bind('<Control-s>', lambda event: save_file(classdict, schedulepath, save_state, invokebutton))
filemenu.add_command(label="Guardar Como...     Ctrl+Alt+S",
                     command=lambda: save_as_file(classdict, save_state, invokebutton, filename, schedulepath))
root.bind('<Control-Alt-s>', lambda event: save_as_file(classdict,
                                                        save_state, invokebutton, filename, schedulepath))
filemenu.add_separator()
filemenu.add_command(label="Salir", command=lambda: on_exit(root))

menubar.add_cascade(label="Archivo", menu=filemenu)

showmenu = tk.Menu(menubar, tearoff=0)
showmenu.add_checkbutton(label='Titulo Horario', onvalue=1, offvalue=False,
                         variable=show_title, command=lambda: update_title_show(show_title, title_label))
showmenu.add_checkbutton(label='Tiempos de Clase', onvalue=1, offvalue=False,
                         variable=show_timestamp, command=lambda: update_timestamp_show(show_timestamp, classdict))

menubar.add_cascade(label="Mostrar", menu=showmenu)

newclassButton = tk.Button(root, text='Añadir Materia',
                           command=lambda: open_class_window(canvas, classdict, reference_channel))
...
credits_display.place(rely=0, relx=0.16)

urosario_fetch = tk.Button(root, text='Añadir de la web...',
                           command=lambda: UrosarioPopUp(canvas, classdict, reference_channel))
urosario_fetch.place(rely=0, relx=0.75, anchor='n')

label = tk.Label(root, text='Total Créditos:', font=('ComicSans', 12))
label.place(rely=0, relx=0)
label = tk.Label(
    canvas, text=' Lunes        Martes       Miércoles     Jueves       Viernes      Sábado', font=('ComicSans', 16))
